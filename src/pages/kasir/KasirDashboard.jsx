...
export const KasirDashboard = () => {
  ...
  const handleConfirmDP = async () => {
    ...
    await changeStatus(selectedOrderDP.id, 'pending', {
      dp_amount: Number(dpInput) || Number(selectedOrderDP.dp_amount) || 0,
  ...
  return (
    <>
      <div className="w-full max-w-[1400px] mb-10 pb-20 md:pb-0 flex flex-col xl:flex-row gap-6 sm:gap-8 overflow-hidden print:hidden">
        ...
        {/* ── Modal: Konfirmasi DP ── */}
        <Modal open={!!selectedOrderDP} onClose={() => setSelectedOrderDP(null)} title="Konfirmasi DP Diterima" size="sm">
          {selectedOrderDP && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-600 font-bold mt-2 uppercase tracking-wide">
                  Estimasi default: 50% dari total tagihan
                </p>
              </div>

              <div className="pt-4 border-t border-slate-100 flex gap-3">
                <button onClick={() => setSelectedOrderDP(null)} className="px-5 py-3.5 bg-slate-100 hover:bg-slate-200 text-[#1A1D1B] font-bold rounded-xl text-sm transition-all">Batal</button>
                <button
        ...
        {/* ── Modal: Pelunasan Akhir ── */}
        <Modal open={!!selectedOrderLunas} onClose={() => setSelectedOrderLunas(null)} title="Konfirmasi Pelunasan & Penyerahan" size="sm">
          {selectedOrderLunas && (
            <div className="space-y-4">
              ...
              <div className="bg-[#EAF4EF] border border-[#a8d5bf] rounded-xl p-4 flex items-start gap-3">
                <CheckCircle2 size={18} className="text-[#347B5A] shrink-0 mt-0.5" />
                <p className="text-[12px] font-semibold text-[#347B5A] leading-relaxed">
                  Pendapatan perusahaan akan terhitung secara resmi pada hari ini setelah pelunasan dikonfirmasi oleh Anda.
                </p>
              </div>

              <div className="pt-4 border-t border-slate-100 flex gap-3">
                <button onClick={() => setSelectedOrderLunas(null)} className="px-5 py-3.5 bg-slate-100 hover:bg-slate-200 text-[#1A1D1B] font-bold rounded-xl text-sm transition-all">Batal</button>
                <button
      ...
      {/* ── Invoice Print View (Tetap Dibawah) ── */}
      {orderToPrint && (
        <div className="hidden print:block w-full max-w-4xl mx-auto p-4 md:p-8 font-sans text-black bg-white min-h-screen">
          <div className="flex justify-between items-end border-b-[3px] border-black pb-6 mb-8">
            <div>
              <h1 className="text-4xl font-black uppercase tracking-wider mb-2 text-black">INVOICE</h1>
              <p className="text-sm font-bold text-gray-800">Trigara Print Management</p>
              <p className="text-sm text-gray-600 italic">"Your Trusted Printing Partner"</p>
            </div>
            <div className="text-right">
              <p className="font-black text-2xl mb-1 text-black">#{orderToPrint.id?.slice(-6).toUpperCase()}</p>
              <p className="text-sm font-bold text-gray-700">{formatDate(orderToPrint.created_at)}</p>
              <div className="mt-2 inline-block border-[2px] border-black px-3 py-1 text-xs font-black uppercase tracking-widest text-black">
                {orderToPrint.status.replace(/_/g, ' ')}
              </div>
            </div>
          </div>

          <div className="mb-10">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 border-b border-gray-300 pb-1">Klien / Customer</p>
            <p className="text-2xl font-black text-black">{orderToPrint.customer_name}</p>
          </div>

          <table className="w-full text-left mb-10 border-collapse">
            <thead>
              <tr className="border-[2px] border-black bg-gray-50 print:bg-transparent">
                <th className="py-4 px-4 text-sm font-black text-black uppercase w-[50%] border-r-[2px] border-black">Deskripsi Pesanan</th>
                <th className="py-4 px-4 text-sm font-black text-black uppercase text-center w-[20%] border-r-[2px] border-black">Jumlah</th>
                <th className="py-4 px-4 text-sm font-black text-black uppercase text-right w-[30%]">Total</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b-[2px] border-l-[2px] border-r-[2px] border-black">
                <td className="py-6 px-4 border-r-[2px] border-black align-top">
                  <p className="font-black text-black text-xl mb-1">{orderToPrint.product_name}</p>
                  <p className="text-sm font-medium text-gray-600 whitespace-pre-wrap">{orderToPrint.notes || '-'}</p>
                </td>
                <td className="py-6 px-4 text-center font-bold text-black text-lg border-r-[2px] border-black align-top">
                  {orderToPrint.quantity} <span className="text-sm text-gray-600">{orderToPrint.product_unit}</span>
                </td>
                <td className="py-6 px-4 text-right font-black text-black text-xl align-top">
                  {formatRupiah(orderToPrint.total_price)}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="flex justify-end mb-16">
            <div className="w-full md:w-1/2 border-[2px] border-black p-5">
              <div className="flex justify-between items-center text-sm font-bold text-gray-700 mb-3">
                <span>Subtotal Harga</span>
                <span className="text-black">{formatRupiah(orderToPrint.total_price)}</span>
              </div>
              <div className="flex justify-between items-center text-sm font-bold text-gray-700 border-b-[2px] border-gray-300 pb-4 mb-4">
                <span>Uang Muka / DP</span>
                <span className="text-black">{formatRupiah(orderToPrint.dp_amount || 0)}</span>
              </div>
              <div className="flex justify-between items-center text-xl font-black text-black">
                <span>Sisa Tagihan</span>
                <span>{formatRupiah(Number(orderToPrint.total_price) - Number(orderToPrint.dp_amount || 0))}</span>
              </div>
            </div>
          </div>

          <div className="text-center border-t-[2px] border-black pt-10 mt-auto">
            <p className="font-black text-black text-2xl tracking-widest mb-2 uppercase">Terima Kasih</p>
            <p className="text-sm text-gray-600 font-bold mb-1">Harap periksa kembali pesanan Anda sebelum meninggalkan tempat.</p>
            <p className="text-xs text-gray-400 font-mono mt-4">Invoice generated automatically by system :: Trigara Print Management</p>
          </div>
        </div>
      )}
    </>
