...
import {
  ...
  LayoutDashboard, Printer, Package, PackageCheck, PlayCircle, CheckCircle2
} from 'lucide-react';
...
const PRODUCTION_STATUSES = ['cetak', 'finishing', 'packing', 'ready'];
const PAGE_SIZE = 10;
...
export const ProduksiDashboard = () => {
  ...
  const displayName = user?.email?.split('@')[0] || 'Produksi';

  const productionOrders = orders.filter(o => PRODUCTION_STATUSES.includes(o.status));
  const cetakOrders = orders.filter(o => o.status === 'cetak');
  const finishingOrders = orders.filter(o => o.status === 'finishing');
  const packingOrders = orders.filter(o => o.status === 'packing');
  const readyOrders = orders.filter(o => o.status === 'ready');

  const totalDone = packingOrders.length + readyOrders.length;
  const progressPercent = Math.min(100, Math.max(0,
    Math.round((totalDone / Math.max(1, productionOrders.length)) * 100)
  ));

  // Active orders for horizontal scroll (cetak + finishing)
  const activeOrders = orders
    .filter(o => ['cetak', 'finishing'].includes(o.status))
    .slice(0, 6);

  // Priority = packing (siap diselesaikan)
  const priorityOrders = packingOrders.slice(0, 4);
  ...
  const statCards = [
    { label: 'Total Produksi', count: productionOrders.length, icon: LayoutDashboard, text: 'text-blue-500', bg: 'bg-blue-50' },
    { label: 'Proses Cetak', count: cetakOrders.length, icon: Printer, text: 'text-cyan-500', bg: 'bg-cyan-50' },
    { label: 'Finishing', count: finishingOrders.length, icon: Package, text: 'text-indigo-500', bg: 'bg-indigo-50' },
    { label: 'Packing / Siap', count: packingOrders.length + readyOrders.length, icon: PackageCheck, text: 'text-[#347B5A]', bg: 'bg-[#EAF4EF]' },
  ];
  ...
  return (
    <div className="w-full max-w-[1400px] mb-10 pb-20 md:pb-0 flex flex-col xl:flex-row gap-6 sm:gap-8 overflow-hidden">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-6 sm:gap-8 overflow-hidden">
        ...
        {/* Hero / Greeting */}
        <div className="relative overflow-hidden bg-[#3d6b5a] rounded-[2rem] p-8 sm:p-10 shadow-xl shadow-[#3d6b5a]/25 flex items-center justify-between border border-[#ffffff10]">
          ...
          <div className="absolute top-10 right-[250px] w-1.5 h-1.5 bg-white/15 rounded-full" />

          <div className="relative z-10 w-full flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
              <p className="text-white/80 font-bold text-[10px] tracking-widest uppercase mb-2">Workspace Petugas Produksi</p>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-2 leading-tight">
                Pantau & Kelola<br />Proses Produksi
              </h2>
              <p className="text-white/60 font-semibold text-sm mt-1">
                {productionOrders.length} pesanan sedang berjalan
              </p>
            </div>
            <button
              onClick={() => navigate('/produksi/proses')}
            ...
            >
              <span className="text-[14px]">Proses Produksi</span>
              <div className="w-7 h-7 bg-[#1A1D1B] rounded-full flex items-center justify-center text-white">
        ...
        {activeOrders.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => navigate('/produksi/proses')}
                className="flex items-center gap-1.5 text-[12px] font-bold text-[#3d6b5a] hover:underline"
              >
            ...
            <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 hide-scrollbar snap-x px-2 -mx-2 pt-2">
              {activeOrders.map(o => (
                <div
                  key={o.id}
                  className="min-w-[280px] max-w-[280px] bg-white rounded-2xl p-5 shadow-[0_4px_20px_rgb(0,0,0,0.04)] border border-slate-100 snap-start flex flex-col group hover:border-[#3d6b5a]/40 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden"
                  onClick={() => setSelectedOrder(o)}
                >
                  <div className={`absolute top-0 left-0 w-full h-[5px] transition-colors ${o.status === 'finishing' ? 'bg-indigo-400' : 'bg-cyan-400'}`} />
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-100">
                    ...
                    <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-[#3d6b5a] group-hover:bg-[#3d6b5a] group-hover:text-white transition-colors border border-slate-100 group-hover:border-[#3d6b5a]">
        ...
        <div id="tableArea" className="bg-white rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-50 overflow-hidden flex flex-col flex-1">
          <div className="p-5 md:p-6 border-b border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <h3 className="font-extrabold text-[#1A1D1B] text-lg w-full sm:w-auto">Daftar Produksi</h3>
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <div className="relative w-full sm:w-[220px]">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border border-transparent focus:bg-white focus:border-[#3d6b5a] focus:ring-4 focus:ring-[#3d6b5a]/10 outline-none text-[13px] font-semibold text-[#1A1D1B] placeholder-slate-400 transition-all"
                />
              </div>
              <div className="relative w-full sm:w-[170px]">
                ...
                <select
                  ...
                  className="pl-10 pr-10 py-2.5 rounded-xl bg-slate-50 border border-transparent focus:bg-white focus:border-[#3d6b5a] focus:ring-4 focus:ring-[#3d6b5a]/10 outline-none text-[13px] font-bold text-[#1A1D1B] appearance-none cursor-pointer w-full transition-all"
                >
          ...
          <div className="flex-1 w-full relative">
            {loading ? (
              <div className="flex items-center justify-center h-48">
                <div className="w-10 h-10 border-4 border-slate-100 border-t-[#3d6b5a] rounded-full animate-spin" />
              </div>
            ) : paginated.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48">
                <div className="text-4xl mb-3 opacity-50">🖨️</div>
                <p className="font-bold text-[15px] text-[#1A1D1B]">Tidak ada pesanan produksi</p>
                    ...
                    {paginated.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <p className="text-[11px] font-semibold text-[#3d6b5a] mt-0.5">{order.quantity} {order.product_unit}</p>
                        </td>
                        <td className="px-6 py-4"><div className="scale-90 origin-left"><StatusBadge status={order.status} /></div></td>
                        <td className="px-6 py-4">
                          <button
                            onClick={() => setSelectedOrder(order)}
                            className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 hover:border-[#3d6b5a] hover:text-[#3d6b5a] hover:bg-[#EAF4EF] transition-all"
                          >
      ...
      <div className="w-full xl:w-[320px] 2xl:w-[340px] shrink-0 space-y-6 flex flex-col">
        ...
        <div className="bg-white rounded-[2rem] p-7 flex flex-col items-center justify-center text-center shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-50 relative">
          ...
          <div className="relative w-32 h-32 mb-6">
            <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="46" stroke="#3d6b5a" strokeWidth="8" fill="none"
                strokeDasharray={`${progressPercent * 2.89} 289`} strokeLinecap="round"
                className="transition-all duration-1000 ease-out" />
            </svg>
            <div className="absolute inset-0 m-auto w-[68px] h-[68px] rounded-full overflow-hidden bg-emerald-50 p-1 border-[3px] border-white shadow-lg shadow-black/5">
            ...
            <div className="absolute top-0 right-0 bg-[#3d6b5a] text-white text-[10px] font-bold px-2 py-0.5 rounded-full border-2 border-white shadow-sm z-10">
              {progressPercent}%
            </div>
          </div>
          ...
          <p className="text-[12px] text-slate-400 font-semibold leading-relaxed max-w-[200px]">
            Selesaikan produksi hari ini dengan tepat waktu!
          </p>

          <div className="w-full mt-6 bg-[#EAF4EF] rounded-2xl p-5 border border-[#3d6b5a]/10 flex items-center justify-between shadow-sm">
            <div className="text-left">
              <p className="text-[10px] font-extrabold text-[#347B5A] uppercase tracking-widest mb-1">Sudah Selesai</p>
              <p className="text-[13px] font-bold text-[#1A1D1B]">{packingOrders.length + readyOrders.length} Order</p>
            </div>
        ...
        <div className="bg-white rounded-[2rem] p-7 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-50 relative flex-1 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-extrabold text-[#1A1D1B] text-[16px]">Siap Packing</h3>
          ...
          <div className="space-y-4 flex-1">
            {priorityOrders.length === 0 ? (
              <div className="flex flex-col flex-1 items-center justify-center text-center py-6 opacity-60">
                ...
                <p className="text-[13px] font-bold text-slate-500">Tidak ada packing tertunda!</p>
              </div>
              ...
              <div key={o.id} className="flex items-center justify-between group gap-2 py-1">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 truncate tracking-wide">{o.product_name?.substring(0, 18)}</p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedOrder(o)}
                  className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-100 text-[#3d6b5a] text-[10px] font-bold tracking-widest uppercase hover:bg-[#EAF4EF] hover:border-[#EAF4EF] transition-all"
                >
                  Cek
                </button>
              </div>
          ...
          {priorityOrders.length > 0 && (
            <button
              onClick={() => navigate('/produksi/proses')}
              className="w-full mt-6 bg-slate-50 hover:bg-slate-100 text-[#3d6b5a] font-bold text-[12px] py-3 rounded-xl transition-colors tracking-wide"
            >
        ...
        {selectedOrder && (
          <div className="space-y-4">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button onClick={() => { setSelectedOrder(null); navigate('/produksi/proses'); }} className="px-5 py-2.5 bg-[#3d6b5a] text-white font-bold rounded-xl text-sm hover:bg-[#346050] transition-colors">Ke Proses</button>
            </div>
          </div>
        )}
      </Modal>
